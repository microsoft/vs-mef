...
%ROBOCOPY% "%~dp0.." "%PrivateAssemblies%" Microsoft.VisualStudio.Composition*.dll Microsoft.VisualStudio.Composition*.pdb

@ECHO Invalidating MEF cache...
for %%i in (devenv VSWinExpress) do (
    IF EXIST "%COMMONIDE%\%%i.exe" "%COMMONIDE%\%%i.exe" /clearCache
