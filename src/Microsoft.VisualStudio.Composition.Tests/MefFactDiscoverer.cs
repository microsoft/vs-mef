namespace Microsoft.VisualStudio.Composition.Tests
{
    ...
    public class MefFactDiscoverer : IXunitTestCaseDiscoverer
    {
        ...
        public IEnumerable<IXunitTestCase> Discover(ITestFrameworkDiscoveryOptions discoveryOptions, ITestMethod testMethod, IAttributeInfo factAttributeInfo)
        {
            ...
            {
                parts = GetNestedTypesRecursively(testMethod.TestClass.Class.ToRuntimeType()).Where(t => (!t.IsAbstract || t.IsSealed) && !t.IsInterface).ToArray();
            }

            if (factAttribute.CompositionVersions.HasFlag(CompositionEngines.V1))
            {
                yield return new MefTestCommand(this.diagnosticMessageSink, methodDisplay, testMethod, CompositionEngines.V1, parts, factAttribute.Assemblies, factAttribute.InvalidConfiguration);
            }

            if (factAttribute.CompositionVersions.HasFlag(CompositionEngines.V2))
            {
                yield return new MefTestCommand(this.diagnosticMessageSink, methodDisplay, testMethod, CompositionEngines.V2, parts, factAttribute.Assemblies, factAttribute.InvalidConfiguration);
            }
            ...
            if ((factAttribute.CompositionVersions & CompositionEngines.V3EnginesMask) == CompositionEngines.Unspecified)
            {
                if (!factAttribute.NoCompatGoal)
                {
                    ...
                    yield return new SkippableFactDiscoverer.SkippableFactCommand(this.diagnosticMessageSink, methodDisplay /* TODO: + " V3" */, testMethod, "Test does not include V3 test.");
                }
            }
            else
            {
                var v3DiscoveryTest = new MefV3DiscoveryTestCommand(this.diagnosticMessageSink, methodDisplay, testMethod, factAttribute.CompositionVersions, parts ?? new Type[0], factAttribute.Assemblies ?? ImmutableList<string>.Empty, factAttribute.InvalidConfiguration);
                yield return v3DiscoveryTest;

                if (v3DiscoveryTest.Passed && (!factAttribute.InvalidConfiguration || factAttribute.CompositionVersions.HasFlag(CompositionEngines.V3AllowConfigurationWithErrors)))
                {
                    foreach (var configuration in v3DiscoveryTest.ResultingConfigurations)
                    {
                        ...
                        yield return new Mef3TestCommand(this.diagnosticMessageSink, methodDisplay, testMethod, configuration, factAttribute.CompositionVersions, runtime: true);
                    }
                }
            }
        }
