...
of the tests present. It reads the command present in the text file and runs the command in VS.Mefx and the stores
...
If you want to use the current output from the VS.Mefx program to create a new test, you can use the CreateSampleTest() method in the
...
As changes are made in the VS.Mefx program, tests might need to be updated to reflect these changes. It is relatively easy to update
the tests if there are updates in the main VS.Mefx program by changing the TestOverride constant in the TestRunner.cs file. What this
